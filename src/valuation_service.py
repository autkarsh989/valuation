...
def run_neural_valuation_for_company(symbol: str, force_refresh: bool = False) -> Dict[str, Any]:
    """
    Runs valuation pipeline for a single company using Neural Network learned weights.
    """
    fetch_and_store_company_data(symbol, force_refresh=force_refresh)
    record = get_company_full_record(symbol)
    ...
    # Use Neural Network Weighted Ensemble
    neural_ensemble = compute_weighted_ensemble_valuation(
        company_id=symbol,
        market_price=market_price,
        sector=sector,
        scenario_results=scenarios,
...
def run_neural_valuation_for_all(force_refresh: bool = False) -> List[Dict[str, Any]]:
    """
    Runs the Neural Network weighted valuation pipeline for all 20 companies.
    """
    logger.info("Starting bulk Neural Network valuation pipeline for 20 companies...")
    for comp in COMPANIES_20:
        sym = comp["symbol"]
        try:
            run_neural_valuation_for_company(sym, force_refresh=force_refresh)
        except Exception as e:
            logger.error(f"Error running neural valuation for {sym}: {e}")
    return get_all_model_company_summaries()
